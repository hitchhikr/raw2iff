...
void process(PLUGIN_COMMAND *plugin_struct)
{
    ...
    switch(plugin_struct->command)
    {
        case PLUGIN_GET_NAME:
            strcpy(plugin_struct->name, "Red:8 Green:8 Blue:8");
            break;

        case PALETTE_GET_COLOR_BYTES:
            plugin_struct->color_size = 3;
            break;

        case PALETTE_GET_COLOR_VALUE:
            for(i = 0; i < plugin_struct->colors; i++)
            {
                colors_block[i].Red = plugin_struct->entry[(i * 3)];
                colors_block[i].Green = plugin_struct->entry[(i * 3) + 1];
                colors_block[i].Blue = plugin_struct->entry[(i * 3) + 2];
            }
